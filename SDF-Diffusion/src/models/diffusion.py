...
class GaussianDiffusion(nn.Module):
    def __init__(self, loss_type="l1", model_mean_type="eps", schedule_kwargs=None, sdf_weight=0.1):
        super().__init__()
    ...
    def p_losses(self, denoise_fn, x_0, noise=None, denoise_kwargs={}, post_fn=identity):
        ...
        if self.model_mean_type == "eps":
            ...
            loss = self.loss_func(x_0, x_recon)
            rolled_x_0 = th.roll(x_0, 1, 4)
            rolled_x_0[:, :, :, 0] = 0
            mask = (th.abs(x_0) < 1 ) & (th.abs(rolled_x_0) < 1)
            diff = th.diff(x_recon, dim=4)
            diff = F.pad(diff, (1, 0))
            diff = diff[mask]
            diff = diff[diff != 0]
            loss += th.mean(th.abs(th.abs(diff)-1))*self.sdf_weight
