...
class SR3dPreprocessor(trainer.BasePreprocessor):
    ...
    def __call__(self, batch, augmentation=False) -> dict:
        ...
        s.hmr, s.intensity, s.im_y, s.im_x, s.c = batch
        s.im_y = s.im_y.to(self.device, non_blocking=True, dtype=th.float32)
        s.im_x = s.im_x.to(self.device, non_blocking=True, dtype=th.float32)
        s.intensity = s.intensity.to(self.device, non_blocking=True, dtype=th.float32)
        ...
        s.intensity = F.interpolate(s.intensity, s.im_y.shape[2:])
        ...
        if self.do_augmentation and augmentation:
            ...
            for i in range(s.n):
                if random.random() < 0.5:
                    ...
                    outs_intensity.append(s.intensity[i].flip(dims=flip).contiguous())
                else:
                    outs_y.append(s.im_y[i])
                    outs_x.append(s.im_x[i])
                    outs_intensity.append(s.intensity[i])
            s.im_y = th.stack(outs_y)
            s.im_x = th.stack(outs_x)
            s.intensity = th.stack(outs_intensity)
        ...
        if self.patch_size is not None:
            ...
            s.pim_intensity = s.intensity[..., t[0] : t[0] + p, t[1] : t[1] + p, t[2] : t[2] + p]

            
        else:
            s.pim_y = s.im_y
            s.pim_x = s.im_x
            s.pim_intensity = s.intensity
    ...
    def standardize(self, x: th.Tensor, i: int):
        ...
        if self.sdf_clip[i] == 0:
            ...
            if self.new_loss:
                x = x.clamp(-2/(self.sdf_clip[i]-1), 2/(self.sdf_clip[i]-1))
                x = (x - self.mean[i]) / (2/(self.std[i]-1))
            else: 
                x = x.clamp(-2/(self.sdf_clip[i]), 2/(self.sdf_clip[i]))
                x = (x - self.mean[i]) / (2/(self.std[i]))
    ...
    def destandardize(self, x: th.Tensor, i: int):
        ...
        if self.sdf_clip[i] == 0:
            ...
            if self.new_loss:
                x = x * 2/(self.std[i]-1) + self.mean[i]
                x = x.clamp(-2/(self.sdf_clip[i]-1), 2/(self.sdf_clip[i]-1))
            else:
                x = x * 2/(self.std[i]) + self.mean[i]
                x = x.clamp(-2/(self.sdf_clip[i]), 2/(self.sdf_clip[i]))
        return x
    ...
    def step(self, s):
        def denoise_fn(x_t, t):
            if self.gaussian_conditional_augmentation:
                ...
                z_t = s.pim_x
                if not -1 in s.pim_intensity and self.use_intensity:
                    v_t = s.pim_intensity
                    input = th.cat([z_t, v_t, x_t], dim=1)
                else:
        ...
        def denoise_fn_wrapper(model):
            def denoise_fn(x_t, t):
                if self.gaussian_conditional_augmentation:
                    ...
                    z_t = s.im_x
                    if not -1 in s.intensity and self.use_intensity:
                        v_t = s.intensity
                        input = th.cat([z_t, v_t, x_t], dim=1)
                    else:
                        input = th.cat([z_t, x_t], dim=1)
    ...
    def sample(self):
        ...
        with tqdm(total=n, ncols=100, file=sys.stdout, desc="Sample", disable=not self.rankzero) as t:
            for c in self.class_idx_rank:
                ...
                # sample_idx = self.dl_test.dataset.cate_indices[synset][: self.n_samples_per_class]
                #randomly pick a number between 0 and len(self.dl_test.dataset)
                sample_idx = random.randint(0, len(self.dl_test.dataset)-1)
                ...
                path = outdir / f"e{self.epoch:04d}.pt"
                # pcu.save_mesh_vf(str(path), v, f)
                    
                ims = ims[0]
                th.save(ims, str(path))
                # print(ims.shape)
                # v,f,n,_ = skimage.measure.marching_cubes(ims[0], level=0)
                # plt.figure()
                # ax = plt.axes(projection='3d')
                # ax.plot_trisurf(v[:,0], v[:,1], v[:,2], triangles=f, cmap='viridis', antialiased=True)

                # v,f,n,_ = skimage.measure.marching_cubes(ims[1], level=0)
                # plt.figure()
                # ax2 = plt.axes(projection='3d')
                # ax2.plot_trisurf(v[:,0], v[:,1], v[:,2], triangles=f, cmap='viridis', antialiased=True)

                # v,f,n,_ = skimage.measure.marching_cubes(ims[2], level=0)
                # plt.figure()
                # ax3 = plt.axes(projection='3d')
                # ax3.plot_trisurf(v[:,0], v[:,1], v[:,2], triangles=f, cmap='viridis', antialiased=True)
                # plt.show()
